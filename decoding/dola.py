"""
DoLA decoding for normal batched LVLM generation.

This file uses HF model.generate(..., dola_layers=...).

DoLA+PHG does not call this file directly. It uses decoding/stepwise.py.
"""
...
class DoLADecoder:
    def __init__(self, config: Optional[DoLAConfig] = None) -> None:
        self.config = config or DoLAConfig()
...
def generate_dola_samples(
    ...
    **prepare_kwargs: Any,
    ...
    return DoLADecoder(config).generate_samples(
        wrapper=wrapper,
        samples=samples,
        image_key=image_key,
